...
| Aplicar Addendum a Project existente | Incorporar el alcance de una Quotation/Addendum Ganada a un Project **ya existente**, sin crear Project (`apply_addendum_to_project`) — contrato de `pmo` Change Control (issue #59, ADR-0019/0020). **Solo acepta addendas canónicas `ROOT-ADD-NN`** (una Quotation de grupo normal no se aplica; modelo `1 Project = 1 raíz + N addendas`); exige que el `project` coincida con el Project raíz resuelto; nunca crea Project (exclusión estructural del auto-Project en 3 capas). Tras materializar+asociar, **recomputa y sincroniza** la economía autorizada del Project | `utils/project.py`, `utils/addendum.py`, `utils/project_economics.py` | Activo |
