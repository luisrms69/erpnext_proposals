...
| Evaluación Económica — financiamiento CAPEX (Fase 2B) | **Costo de fondeo** de la adquisición del CAPEX (nuestro interés + comisiones, **no** una tasa al cliente; el **principal no es costo**). Capa **aditiva** sobre 2A: se conservan `total_cost`/`margin` y se añaden `financial_cost`, `total_cost_with_financing`, `margin_after_financing` (+ %). Amortización **PMT vencido mensual** (`_amortize`/`_effective_financing`): cuota fija (`P·r/(1−(1+r)⁻ⁿ)` o `P/n` si tasa 0), última cuota cierra saldo en 0, comisiones en `Mes 0`, interés de cada cuota en su periodo. `financed_amount` por defecto = costo externo del grupo CAPEX; **financiar > ese costo = error**. El horizonte (`economic_horizon_months`) se **extiende** si el financiamiento supera el plazo (`warnings: financing_extends_horizon`) sin extender MRC. **Fail-closed** (financiado ≤0/>CAPEX, plazo ≤0, tasa <0, comisiones <0, financiamiento sin CAPEX → `EconomicEvaluationError`) + **invariantes 2B** en `_assert_reconciled`. **Defaults de Company = solo precarga** (`_default_financing` al activar); tras eso la Quotation es autoritativa — el motor lee tasa/plazo del documento **sin fallback** a la Company (una **tasa 0% explícita es válida**). **Freeze por inmutabilidad** (campos `allow_on_submit=0`): al submit quedan fijos y la evaluación histórica es estable. UX: sección **solo si hay CAPEX** (revelación progresiva en `quotation.js`), memoria de amortización en la pestaña, KPI financiero en el PF. | `utils/economic_calendar.py` (`_amortize`, `_effective_financing`), `utils/quotation.py` (`_default_financing`), `public/js/quotation.js`, `print_format/rentabilidad_estimada/`, `doctype/proposal_settings/` | Activo |
