...
| Aplicar Addendum a Project existente | Incorporar el alcance de una Quotation/Addendum Ganada a un Project **ya existente**, sin crear Project (`apply_addendum_to_project`) — contrato de `pmo` Change Control (issue #59, ADR-0019). Para una **addenda** (`<ROOT>-ADD-<NN>`) exige que el `project` recibido **coincida** con el Project raíz resuelto; una addenda **nunca crea Project** (exclusión estructural del auto-Project en 3 capas: gate de encolado, job y `create_project_from_quotation` fail-closed) | `utils/project.py`, `utils/addendum.py` | Activo |
