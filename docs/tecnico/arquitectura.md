...
| `Proposal Phase` | Catálogo de fases (`phase_code`, `phase_name`, `sequence`, **`color`** Color, **`planned_duration_days`** Int) | Referenciado por `phase` en Scope Item / Quotation Scope Item | El orden en propuesta/reportes/Tasks usa `sequence`; el display usa `phase_name`. `color` y `planned_duration_days` (Tema 3) son atributos de planificación que se **congelan** en la Task padre de fase al generar el Project (campos nativos `Task.color`/`Task.duration`); cambiarlos después no altera Projects ya creados. Helpers en `utils/phase.py` (`phase_label`, `order_phases`, jinja methods) |
