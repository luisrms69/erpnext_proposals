...
| Contrato canónico de addendas | **Semántica única** de las addendas `<ROOT>-ADD-<NN>` (ADR-0019 §5–§6): reconocimiento/parseo/resolución de ROOT, namespace reservado fail-closed, resolución del Project raíz (desde la Ganada vigente del ROOT, **nunca** por `Project.project_name`) y **creación atómica** `create_addendum_quotation` (lock `SELECT … FOR UPDATE` **por ROOT** + secuencia + insert en la misma transacción, sin commit; addenda = **delta** sin copiar items/scope/`proposal_project`/template). El resto de la app trata `proposal_group` como clave **opaca**; no hay campos `addendum_of`. | `utils/addendum.py` | Activo |
| Aplicar Addendum a Project existente (apply-split) | Incorporar el alcance de una Quotation/Addendum Ganada a un Project **ya existente**, sin crear Project (`apply_addendum_to_project`) — contrato de `pmo` Change Control (issue #59, ADR-0019 §7.1/0020). **Solo acepta addendas canónicas `ROOT-ADD-NN`** (`1 Project = 1 raíz + N addendas`); exige que el `project` coincida con el raíz resuelto; nunca crea Project. **Apply-split (Change Control v2 B2):** **Fase 1 SIEMPRE** asocia `proposal_project` + sincroniza economía; **Fase 2 SOLO si hay scope ejecutable** (`include_in_proposal OR is_internal_cost_task`) valida y materializa Tasks — una addenda económica-only/solo Required/delta $0 se aplica con 0 Tasks. Sin commit interno (atómico por rollback externo). | `utils/project.py`, `utils/addendum.py`, `utils/project_economics.py` | Activo |
| Gate de venta neta root vs addenda (B1) | En la transición Borrador→En Revisión el gate `net_total>0` aplica **solo** a propuestas root (grupo normal); una addenda canónica puede formalizarse con `net_total==0`. `proposal_template`/`proposal_cost_center` siguen obligatorios para ambas (ADR-0019 §7). | `utils/workflow_validations.py` | Activo |
| Versionado de Required Items (B3) | `create_new_proposal_version` copia `required_items` conservando **solo** el input semántico (`item`/`qty`/`uom`/`auto_generated`); los snapshots congelados los repuebla el freeze al re-formalizar (ADR-0019 §7.4). | `utils/proposal_versioning.py` | Activo |
| Huella canónica del delta de addenda (ADR-0019 §7.2) | `get_addendum_delta_fingerprint(quotation) -> str` (read-only, server-side, **no** whitelisted): SHA-256 de un payload canónico (revenue/labor/external) del delta **semántico congelado** —incluye materialización (`item_code`/`phase`/`include_in_proposal`/`is_internal_cost_task`) y economía congelada de líneas vendidas (ADR-0020)—. Fail-closed (addenda canónica + `docstatus>=1` + `assert_economic_snapshot_complete`; dependencias inválidas lanzan; deps como conjunto). Ignora IDs técnicos y preserva multiplicidad: una nueva versión con el mismo delta produce la misma huella. Consumo futuro por `pmo` (registro + guard de `apply`); **no** implementado aún en `pmo`. | `utils/addendum.py` | Activo |
| Contrato económico Project ↔ Quotations (ADR-0020) | **Autorizado vigente** de un Project = **Original(root) + Σ(addendas aplicadas)** por magnitud (revenue/cost/margin/labor/external; `margin_pct` **derivado**), fuente única = motor de Evaluación Económica (`totals`, **sin financiamiento**; no reimplementa fórmulas). `get_project_authorized_economics` (lectura) + `sync_project_authorized_cost` que espeja `Project.estimated_costing` (`set_value(update_modified=False)`, idempotente, sin `save()`/`update_costing`). Raíz resuelta por `proposal_project` (Ganada+submitted+no superseded+grupo normal, **exactamente una** o fail-closed); addendas aplicadas por `proposal_project==project`. **Fail-closed** ante snapshot incompleto (guard canónico `assert_economic_snapshot_complete`, reusado por submit/workflow/economía). **Multimoneda (ADR-0024):** el ingreso se normaliza a moneda base con el `conversion_rate` nativo; solo se detiene si una Cotización en moneda distinta a la base carece de `conversion_rate`. No depende de `pmo`. | `utils/project_economics.py`, `utils/project.py`, `utils/quotation.py` | Activo |
| Automatización al ganar (issue #39, Fase 1) | Al transicionar a **`Ganada`** (detección en la transición de workflow), acciones **independientes** por Company (`Proposal Settings`, sección «Automatización al ganar la propuesta», **todas OFF por defecto**, sin switch maestro), **encoladas después del commit** (`enqueue_after_commit=True`, nunca en `validate`): (1) **Crear Project** (`auto_create_project_on_won`) reutilizando `create_project_from_quotation` (idempotente, fail-soft, sin Project parcial; botón manual = respaldo); (2) **Notificar por correo** (`send_won_notification_email` + `won_notification_email` obligatorio + `won_notification_email_template` opcional) vía `frappe.sendmail`/Email Queue nativo, con la **API nativa `Email Template.get_formatted_email`** cuando hay plantilla y un fallback propio si no. Idempotencia nativa (gating por transición + `enqueue(job_id, deduplicate)`); jobs como el usuario que marca Ganada (sin elevar privilegios). Un fallo de una acción no impide la otra. | `utils/workflow_validations.py` (`_maybe_enqueue_auto_project`, `_maybe_enqueue_won_notification`), `utils/project.py` (`auto_create_project_on_won`), `utils/won_notification.py`, `doctype/proposal_settings/` | Activo |
...
| Loader de catálogos | Carga genérica e idempotente por ruta externa de la **capa editorial/de presentación** (caps v12, **depuración de fuentes de verdad**, ADR-0023): Proposal **Sections**, **contenido editorial de Items** (methodology/expected result/scope limit/service text/validity/min unit/service hours — **el loader NUNCA crea Items**: el registro/flags funcionales se administran en Desk; si el Item no existe se reporta en `pending`), **Letter Heads**, **Print Formats + versionamiento declarativo** (deshabilitar anterior + changelog) y **`clear_fields`** (solo sections/items/letter_heads). **YA NO** siembra los maestros funcionales (Proposal Templates y su estructura, Scope Items y relaciones, Proposal Phases y Tags, registro/flags de Item, Payment Terms/Templates, Designations, Skills, `economic_behavior_rules`): se administran EXCLUSIVAMENTE en Desk. `capabilities()` expone `no_functional_master_writes` y ya no expone `designations_skills`/`phase_tags`/`payment_terms`/`economic_behavior_rules`/`scope_pmo_planning` | `catalog_data/catalog_loader.py` | Activo |
| Reporte de rentabilidad | Fuente de datos compartida entre UI y Print Format. Modelo **aditivo** (ADR-0017): ingresos (Items vendidos) − costo externo de compra (Items vendidos/requeridos **comprables**) − costo de esfuerzo (Scope Items). Costo externo vía pricing **nativo** (`utils/item_cost.resolve_external_cost`: gate `is_purchase_item` → `get_item_price` → `last_purchase_rate` → `valuation_rate`), **normalizado a moneda base con FX nativo y congelado con trazabilidad de origen** (`proposal_frozen_cost_*` en Quotation Item y `frozen_cost_*` en Required Item; ADR-0024). El reporte opera en base y **presenta en `Quotation.currency`** vía `conversion_rate` nativo | `report/profitability_estimate/`, `utils/item_cost.py`, `utils/currency.py` | Activo |
...
| Evaluación Económica — sensibilidad de duración (Fase 2C parcial) | Responde «¿cómo se comporta la **misma** propuesta a otra duración?» **en memoria**, sin persistir ni duplicar la Quotation. `get_duration_sensitivity(quotation_name, down_months, up_months)` (**whitelisted** + método jinja) evalúa `base − X` / `base` / `base + Y` con `base = proposal_contract_term_months` (un solo `D` reconocible; ventana por defecto **± 33.33 %** del plazo base configurable en `Proposal Settings` —`duration_sensitivity_down_percent`/`_up_percent`, lector defensivo con `has_field`—, **sin constante fija en la lógica**). `get_economic_evaluation` se refactoriza en wrapper + núcleo `_evaluate_doc(doc, *, term, scope_scale, base_term)` (**base sin regresión**: `term = base_term`). **Se sensibiliza el mismo proyecto, no las ventas:** para recurrentes el **total contractual se ancla al plazo base y se redistribuye** (`rec_scale = ocurrencias(base)/ocurrencias(D)` en `_add_line`), de modo que **TCV, ingreso total, MRC contractual y costo externo total NO cambian**; NRC/CAPEX ocurren una vez (constantes). El esfuerzo se redistribuye con `scope_scale = D/base_D` conservando horas/tarifa/**costo total de labor** → cambia solo la **distribución mensual** y el **pico h/mes**. El **financiamiento** re-amortiza sobre `D` (ADR-0018 §15: sin plazo independiente) → **único costo dependiente del tiempo** y único driver del margen final; el **margen operativo es constante**. Guardas del reescalado lineal: milestones solo reposicionan, piso **1 día**, redondeo determinista `_round_days`. El modelo añade `hours_by_month`/`peak_hours_per_month` (aditivos). **Integrado al PF `Rentabilidad Estimada`** (dos secciones finales generadas automáticamente: «Resumen financiero» NRC/MRC/CAPEX/Total + KPIs con etiquetas precisadas —`Margen operativo`, `Costo operativo`, `TCV / Valor total del contrato`—, y «Análisis de sensibilidad» con columna **MRC equiv./mes** = MRC contractual / D); el **botón/Dialog «Sensibilidad de duración» se retiró** de `quotation.js`. Defaults en `Proposal Settings` (Sección «Sensibilidad de duración», ±33.33 %). **Sin fixtures, sin Custom Fields; contingencia fuera.** Ver ADR-0018 §16. | `utils/economic_calendar.py` (`get_duration_sensitivity`, `_default_sensitivity_window`, `_sensitivity_percents`, `_evaluate_doc`, `_add_line`, `_scope_effort`, `_round_days`), `print_format/rentabilidad_estimada/`, `doctype/proposal_settings/`, `hooks.py` (jinja) | Activo |
...
| Paquetes de alcance (issue #55) | **No es DocType ni motor nuevo:** un paquete es un `Item` organizativo (`is_sales_item=0`, normalmente `is_purchase_item=0`) que agrupa Scope Items por la N:M y se incorpora como **Required Item** (materializa `source_type="required"` por el flujo normal). Automatización "vendido→paquete" = `required_item_rules` existente. Clasificación por **inferencia solo-UX** (`scope_package_query`: no vendible + no comprable + ≥1 Scope Item habilitado); el grid `required_items` conserva su doble propósito (no se filtra). UX: acción **«Agregar paquete de alcance»** (grupo Propuesta, solo Borrador) → diálogo que agrega filas a `required_items` con dedup + preview. Solapamiento = ocurrencias por `(source_row, scope_item)` (sin dedup global). Ver ADR-0016/0017 (enmienda 2026-09). | `utils/scope_item_links.py` (`scope_package_query`, `get_scope_package_scope_items`), `public/js/quotation.js` | Activo |
| Inferencia de Project Type (issue #55, commit 2) | Custom Field opcional `Item.proposal_project_type` (Link → Project Type) en los paquetes. `create_project_from_quotation` infiere `Project.project_type` de los paquetes en **`required_items`** (nunca de `Quotation.items`): 0→sin tipo; 1 distinto→asigna; **≥2 distintos→bloquea** (fail-closed, sin Project a medias). Lector defensivo `has_field`. | `utils/project.py` (`_resolve_project_type`), `fixtures/custom_field.json`, `hooks.py` | Activo |
| Gestión de Compras como paquete (issue #55, commit 3) | Sustituye `Proposal Settings.default_procurement_scope_item` (Scope Item por-ocurrencia) por `default_procurement_package_item` (Link → Item). `_autoload_procurement_package` agrega el paquete **una vez** si hay ≥1 comprable aplicable (vendido o requerido, `is_purchase_item=1`, sin `proposal_skip_procurement`), disparo por diff, **sin auto-remove**. `_applicable_scope_items` queda solo con la N:M. **Migración:** reconfigurar por Company; sin automigración. Enmienda ADR-0017. | `utils/quotation.py` (`_autoload_procurement_package`, `_applicable_purchasables`), `doctype/proposal_settings/` | Activo |
...
| UX «Contenido de propuesta» + «Vista previa» | Edición y revisión de `proposal_sections` con Frappe nativo | Pestañas custom `proposal_content_tab` (aloja `proposal_sections`) y `proposal_preview_tab` (HTML field `proposal_preview_html`). Grid simplificado (in_list_view = `title`+`hide_title`; `sequence`/`proposal_section`/`is_executive_summary` **hidden**). **Orden SEMÁNTICO por `sequence` (no espejo de `idx`):** el Print Format comercial ancla el bloque sintético Plan/Entregables/**Inversión** en `sequence == 500` (`<500` va ANTES del bloque, `>=500` DESPUÉS). El usuario reordena por **arrastre libre** (`idx`, puede cruzar la frontera 500); en Borrador `on_quotation_validate` → `_sync_proposal_section_sequence` reasigna `sequence` por **PUNTO MEDIO entre vecinos** desde el orden visual (una fila que ya respeta el orden se conserva; si no hay hueco entero, renumera preservando orden y lado de 500), y ordena las filas por `sequence` para que `idx` siga al orden canónico. **Nunca `sequence = idx`**; nunca `0`/vacío; ad-hoc recibe valor en rango entre sus vecinos. **Vista previa** = render de cliente (`render_proposal_preview`) que lee EXCLUSIVAMENTE `frm.doc.proposal_sections` (orden `idx` actual), sanitiza con `frappe.dom.remove_script_and_style` (la vía del Text Editor) y se re-renderiza en refresh, ediciones y **al activar la pestaña** (refleja arrastres sin guardar). Sin segunda copia, sin Page, sin llamadas al servidor. **Preview/PDF de SOLO LECTURA:** los botones de Vista previa/Descargar NUNCA reconstruyen la narrativa — solo persisten ediciones pendientes y renderizan (`generate_pdf`); el `resync_scope_from_catalog` (acción explícita de scope/costos) **ya no re-pulla `proposal_sections`** desde el Template (ADR-0022: la narrativa materializada es la fuente única) | `public/js/quotation.js`, `utils/quotation.py` (`_sync_proposal_section_sequence`, `resync_scope_from_catalog`), `doctype/proposal_quotation_section/`, `fixtures/custom_field.json` | Activo |
...
| **Costo externo** | `Quotation Item.proposal_frozen_cost_*` · `Proposal Required Item.frozen_cost_*` (rate **base** + moneda base + importe/moneda origen + FX) | Pricing nativo normalizado a base (`resolve_external_cost`, ADR-0024) |
...
### Costo externo multimoneda ([ADR-0024](../adr/0024-costo-externo-multimoneda.md))

El costo externo de compra es genérico y multimoneda (sin conocimiento de ninguna app consumidora;
frontera `Item + contexto económico → resolver → costo`). Tres monedas **nunca mezcladas**:

- **Moneda canónica del análisis económico interno = `Company.default_currency` (base).** Costo, margen y
  snapshots viven en base. El costo laboral (salarios, `Proposal Cost Matrix`) ya está en base.
- **Moneda fuente** = la de la Buying Price List (o de los fallbacks nativos).
- **Moneda de presentación = `Quotation.currency`** (campo **nativo**; sin selector paralelo). Los
  importes base se presentan con el `conversion_rate` **nativo** (base → presentación = `1/conversion_rate`).

**Resolver único** `resolve_external_cost(item_code, uom, transaction_date, company) -> ExternalCost`
(`utils/item_cost.py`), usado por **Items vendidos (`items`) y Required Items (`required_items`) por
igual**. Devuelve `amount` (normalizado a base), `source`, `source_amount`, `source_currency`,
`normalized_currency`, `exchange_rate`, `exchange_date`.

- **Jerarquía:** Buying Item Price → `last_purchase_rate` → `valuation_rate` → `sin_costo`
  (`last_purchase_rate`/`valuation_rate` ya son base).
- **Múltiples Buying Price Lists** (se elimina "una Company = una lista"): resolución determinista
  (`_resolve_buying_price`) — listas de compra con precio vigente (UOM/vigencia nativas) → desempate por
  `Buying Settings.buying_price_list` → única → fallbacks. Varias aplicables sin criterio →
  **`ambiguo_price_list`** (nunca se elige la primera).
- **FX nativo ERPNext** (`erpnext.setup.utils.get_exchange_rate` sobre `Currency Exchange`, dirección
  `for_buying`; fundación en `utils/currency.py`). **Sin tasa → `sin_tipo_cambio`** (nunca se asume 1.0);
  no hay DocType/tabla/tasas FX propias.
- **Fecha económica** = `Quotation.transaction_date`.
- **Trazabilidad del origen:** el freeze conserva importe base **y** importe/moneda origen + FX
  (`*_source_amount`/`*_source_currency`/`*_exchange_rate`) → un costo congelado es auditable
  (`10 USD × 18 = 180 MXN`), nunca un `180` sin contexto.
- **Fail-closed:** `sin_tipo_cambio` y `ambiguo_price_list` permiten editar el Borrador pero
  `assert_economic_snapshot_complete` **bloquea la formalización** (no se degradan a 0).
- **Presentación:** `profitability_estimate` opera en base y presenta en `Quotation.currency` vía
...
- **Fuera de alcance (hoy):** la **presentación del Calendario Económico en `Quotation.currency`** (el
  calendario se expresa en moneda base); y el FX por periodo. Diferidos en ADR-0024 §5.
