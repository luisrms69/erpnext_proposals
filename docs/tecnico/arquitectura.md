...
| UX «Contenido de propuesta» + «Vista previa» | Edición y revisión de `proposal_sections` con Frappe nativo | Pestañas custom `proposal_content_tab` (aloja `proposal_sections`) y `proposal_preview_tab` (HTML field `proposal_preview_html`). Grid simplificado (in_list_view = `title`+`hide_title`; `sequence`/`proposal_section`/`is_executive_summary` **hidden**). **Orden nativo por arrastre (`idx`)**: en Borrador, `on_quotation_validate` → `_sync_proposal_section_sequence` alinea `sequence` (orden canónico de PF/`get_sections_snapshot`) con el orden visual **solo cuando difieren** (preserva valores del Template si no hubo reorden; no expone `sequence`). **Vista previa** = render de cliente (`render_proposal_preview` en `quotation.js`) que lee EXCLUSIVAMENTE `frm.doc.proposal_sections` en orden `idx`, sanitiza con `frappe.dom.remove_script_and_style` (la misma vía del Text Editor) y se re-renderiza en refresh, ediciones de fila y **al activar la pestaña** (refleja arrastres sin guardar). Sin segunda copia, sin Page, sin llamadas al servidor | `public/js/quotation.js`, `utils/quotation.py`, `doctype/proposal_quotation_section/`, `fixtures/custom_field.json` | Activo |
