...
| Loader de catálogos | Carga genérica e idempotente por ruta externa: Proposal Phases, Sections, Items (+ campos editoriales), Scope Items (incluye la relación N:M `"erpnext_items": [...]` — lista de Item codes; clave ausente = no toca, presente = sincroniza, vacía = limpia; valida duplicados y reporta Items inexistentes en `pending`), Proposal Templates, Print Formats, **Payment Terms / Payment Terms Templates** (ADR-0006), **`is_purchase_item`** en Items (clave ausente = no toca; permite declarar servicios no-comprables) y **`economic_behavior_rules`** por Company en `Proposal Settings` (`_seed_economic_behavior_rules`, ADR-0018: identidad `(company, source_type, source)`, idempotente, no borra reglas no declaradas ni toca otros campos de Proposal Settings) | `catalog_data/catalog_loader.py` | Activo |
