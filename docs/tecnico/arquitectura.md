...
| `Proposal Settings` | Configuración de precarga **por `Company`** (ADR-0017 Fase 1 bis; **no** Single) | `company` (Link → Company, reqd/único, autoname `field:company`); child `Proposal Required Item Rule` (`required_item_rules`); `default_procurement_scope_item` (Link → Scope Item) | Editable por `System Manager` y `Proposals Manager`. **Máximo uno por Company** (autoname + `_assert_unique_per_company`). Resolución **estricta** por `quotation.company` (`_proposal_settings(company)`): sin settings para la Company → sin precarga ni abastecimiento; **sin fallback global**. DocType de lista, no pantalla Single |
