...
| Frappe `File` | Almacenamiento de PDFs generados | `utils/quotation.py` — `attach_proposal_pdfs()` | PDF público: Propuesta Comercial; privado: Rentabilidad Estimada. `get_proposal_documents_status()` (whitelisted) comprueba su existencia real (por prefijo de nombre, por el hash de `save_file`) para que el botón `Propuesta` (JS) oculte la acción de **re-generar** cada documento oficial una vez adjunto, sin quitar el acceso a los ya generados |
